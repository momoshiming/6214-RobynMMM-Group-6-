...
cor_heat2 <- ggplot(data = melted_cormat, aes(Var2, Var1, fill = value))+
  geom_tile(color = "white")+
  scale_fill_gradient2(low = "blue", high = "red", mid = "white", 
                       midpoint = 0, limit = c(-1,1), space = "Lab", 
                       name="Correlation") +
...
###add total spend
data_week$total_spend <- data_week$facebook_newsfeed_spend
+ data_week$youtube_brand_spend
+ data_week$search_spend
+ data_week$youtube_performance_spend
+ data_week$newspaper_spend
+ data_week$tv_spend

# spend and sales associations
sales_spend <- ggplot(data_week, aes(x=total_spend, y=sales, color=year)) + 
  geom_point()+
  geom_smooth() +
